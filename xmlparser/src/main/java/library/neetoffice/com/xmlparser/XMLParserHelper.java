...
class XMLParserHelper extends DefaultHandler {
    ...
    XMLParserHelper(Object object) throws XMLParserException {
        this.object = object;
    ...
    private void analyzeField(Object object, Field field, String xmlPath) throws XMLParserException {
        ...
        if (element != null) {
            ...
            if (type == Collection.class) {
                try {
                    addCollection(new ArrayList(), object, field, element, xmlPath);
                ...
                try {
                    addCollection(new HashSet(), object, field, element, xmlPath);
                } catch (IllegalAccessException e) {
                    throw new XMLParserException(e.getMessage());
                }
                ...
                try {
                    addCollection(new TreeSet(), object, field, element, xmlPath);
                } catch (IllegalAccessException e) {
                    throw new XMLParserException(e.getMessage());
                }
    ...
    private void addCollection(Collection collection, Object object, Field field, Element element, String xmlPath) throws IllegalAccessException {
        field.set(object, collection);
        final Class cls = getParameterizedType(field);
        if (cls != null) {
            try {
                cls.newInstance();
            } catch (InstantiationException e) {
                throw new XMLParserException(cls.getCanonicalName() + " doesn't have no-arg(default) constructor");
            }
            final String name = Object2StringHelper.getElementName(element, field);
            final String key = xmlPath + name + "_";
            typeTemp.put(key, cls);
    ...
    public void endElement(String uri, String localName, String qName) throws SAXException {
        ...
        if (characters.containsKey(key)) {
            ...
            try {
                addFieldValue(field, object, stringBuffer.toString().replace("\\r", "\r").replace("\\n", "\n"));
            } catch (IllegalAccessException e) {
            }
    ...
    private static Class<?> getParameterizedType(Field f) {
